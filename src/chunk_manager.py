...
class ChunkManager:
    def __init__(self):
        self.chunk_size = config.get("chunking", {}).get("chunk_size", 512)
        self.chunk_overlap = config.get("chunking", {}).get("chunk_overlap", 128)
...
if __name__ == "__main__":
    ...
    cm = ChunkManager()
    file_list = cm.list_full_paths(ocr_structured_path, "*.json", limit=10)
    all_chunk = []
